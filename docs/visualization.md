...
The graph visualization inside the [HTML output](./html.md) is just an example of how to render the `graph.json` knowledge graph (aka KG). There are many ways one can render a graph and we encourage you to discover [yFiles](https://www.yfiles.com/demos) and [Ogma](https://doc.linkurious.com/ogma/latest/) towards enterprise level visualization of KGs.

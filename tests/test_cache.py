...
class TestCacheKey:
    ...
    def test_cache_key_generation(self):
        ...
        key = create_llm_cache_key("test prompt", "gpt-4", 0.7, 100)
        assert isinstance(key, str)
        assert len(key) == 64  # SHA256 hex digest length

    def test_cache_key_consistency(self):
        """Test that same inputs produce same cache key."""
        key1 = create_llm_cache_key("prompt", "model", 0.5, 50)
        key2 = create_llm_cache_key("prompt", "model", 0.5, 50)
        assert key1 == key2

    def test_cache_key_different_for_different_inputs(self):
        """Test that different inputs produce different keys."""
        key1 = create_llm_cache_key("prompt1", "model", 0.5, 50)
        key2 = create_llm_cache_key("prompt2", "model", 0.5, 50)
        assert key1 != key2
...
class TestGetCachedResponse:
    ...
    def test_get_cached_response_not_found(self):
        ...
        result = get_cached_llm_response("nonexistent_key")
    ...
    def test_get_cached_response_found(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("knwler.cache.CACHE_DIR", Path(tmpdir)):
                ...
                cache_file = Path(tmpdir) / "llm" / f"{key}.json"
    ...
    def test_save_to_cache_creates_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("knwler.cache.CACHE_DIR", Path(tmpdir)):
                ...
                save_llm_response_to_cache(key, response, model)

...
async def run_demo(backend: str | None = None):
    ...
    config = Config(backend=backend or "ollama", use_cache=True)
    ...
    rephrased_chunks_path.write_text(
        json.dumps([asdict(chunk) for chunk in rephrased_chunks], indent=2),
        encoding="utf-8",
    )
    ...
    clustered_graph = await cluster_graph(document_graph.graph, config)
    ...
    # Step 9: infer or assign a title
    title = await extract_title(chunks, config)
    console.print(f"[green]✔ Extracted title: {title}[/green]")

    # Step 10: extract a summary
    summary = await extract_summary(chunks, config)
    console.print(f"[green]✔ Extracted summary:[/green]")
    console.print(Markdown(summary))

    # Step 11: save the whole lot

    # for convenience we add the entities found within a chunk
    extended_chunks = []
    document_id = str(uuid4())
    for u in document_graph.chunks:
        entities = [e for e in document_graph.graph.entities if u.id in e["chunk_ids"]]
        relations = [
            r for r in document_graph.graph.relations if u.id in r["chunk_ids"]
        ]
        extended_chunks.append(
            {
                ...
                "text": u.text,
                "rephrase": rephrased_chunks[u.chunk_idx].text,
                "entities": entities,
                "relations": relations,
